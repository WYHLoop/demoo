...
- (instancetype)initWithPresentedViewController:(UIViewController *)presentedViewController presentingViewController:(UIViewController *)presentingViewController
{
    ...
    if (self) {
        ...
        /*
         呈现的视图控制器必须具有UIModalPresentationCustom的modalPresentationStyle，以便使用自定义表示控制器。
         */
...
//这是在演示文稿开始时在演示控制器上调用的第一个方法之一。 调用此方法时，已创建容器View并为演示文稿设置视图层次结构。 但是，尚未检索到呈现的视图。
...
- (void)presentationTransitionWillBegin
{
    ...
    //将呈现的视图控制器视图包装在中间层次结构中，该层次结构将阴影和圆角应用于左上角和右上角。 使用三个中间视图构建最终效果。
